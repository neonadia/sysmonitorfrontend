...
def makeSaaExcutable():
    process = Popen('chmod 777 saa', shell=True, stdout=PIPE, stderr=PIPE)
    process.communicate()
    # printf("saa is excutable now")
    return(0)

def saaRunCustomProcess(CMD):
    # flag = read_flag()
    # while(flag == 1):
    #     flag = read_flag()
    # insert_flag(5)
...
def saaLogOutput():
    process1 = Popen('ps aux | grep "[s]aa" | grep -v "/bin/sh"', shell=True, stdout=PIPE, stderr=PIPE) # run saa in docker container will result in two processes
...
def saaBiosUpdate(inputpath,filepath):
    # flag = read_flag()
    # while(flag == 1):
    #     flag = read_flag()
    # insert_flag(5)
    process = Popen('./saa -l ' + inputpath + ' -c UpdateBios --file ' + filepath, shell=True, stdout=PIPE, stderr=PIPE)
    process.communicate()
    insert_flag(0)
    return(0)

def saaBMCUpdate(inputpath,filepath):
    # flag = read_flag()
    # while(flag == 1):
    #     flag = read_flag()
    # insert_flag(5)
    process = Popen('./saa -l ' + inputpath + ' -c UpdateBmc --file ' + filepath, shell=True, stdout=PIPE, stderr=PIPE)
    process.communicate()
    insert_flag(0)
    return(0)

def saaGetBiosSettings(inputpath):
    # flag = read_flag()
    # while(flag == 1):
    #     flag = read_flag()
    # insert_flag(5)
    process = Popen('./saa -l ' + inputpath + ' -c GetCurrentBiosCfg --file htmlBios --overwrite', shell=True, stdout=PIPE, stderr=PIPE)
    process.communicate()
    insert_flag(0)
    return(0)
    

def saaChangeBiosSettings(inputpath,filepath):
    # flag = read_flag()
    # while(flag == 1):
    #     flag = read_flag()
    # insert_flag(5)
    process = Popen('./saa -l ' + inputpath + ' -c ChangeBiosCfg --file ' + filepath + ' --skip_unknown', shell=True, stdout=PIPE, stderr=PIPE)
    process.communicate()
    insert_flag(0)
    return(0)

def saaCheckOOB(inputpath): # License Key checking
    # flag = read_flag()
    # while(flag == 1):
    #     flag = read_flag()
    # insert_flag(5)
    process = Popen('./saa -l ' + inputpath + ' -c CheckOOBSupport', shell=True, stdout=PIPE, stderr=PIPE)
    ...
    for p in log_lines_trim_parts:
        for i, l in enumerate(p):
            if 'System Name' in l:
                ...
                print(f'({cur_bmc_ip}) Fetching Activation Info using SAA ...')
        for l in p:
...
'''
def saaGetDmiInfo(inputpath):
    process = Popen('./saa -l ' + inputpath + ' -c GetDmiInfo --file DMI --overwrite', shell=True, stdout=PIPE, stderr=PIPE)
    process.communicate()
    return(0)
'''

'''
def saaRemoveFiles(inputtype):
    process = Popen('rm -f ' + inputtype + "*", shell=True, stdout=PIPE, stderr=PIPE)
    process.communicate()
    return(0)
'''

def saaCompBiosSettings():
...
def saaBootOrder(jsonlist,iplist):
...
def saaRedfishAPI(inputpath, API_url):
    ...
    # run redfishAPI    
    process = Popen('./saa -l ' +  inputpath + ' -c RedfishAPI --api ' + API_url + ' --file ' + folder + 'redfish-json --overwrite', shell=True, stdout=PIPE, stderr=PIPE)
    stdout, stderr = process.communicate()
